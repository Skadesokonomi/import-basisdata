...
class FDCCreateSystemAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        fdc_scripts = QgsProcessingParameterEnum('run_scripts', 'Choose which SQL scripts to run', ['{} ... {}'.format(key,self.options[key]['dato']) for key in self.options], allowMultiple=True, defaultValue=[0,1,2,3])
        fdc_scripts.setFlags(adm_database.flags() | QgsProcessingParameterDefinition.FlagAdvanced)
        self.addParameter(fdc_scripts)
        ...
        self.addParameter(fdc_read_role)# -*- coding: utf-8 -*-
...
class FDCCreateSystemAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        fdc_scripts = QgsProcessingParameterEnum('run_scripts', 'Choose which SQL scripts to run', ['{} ... {}'.format(key,self.options[key]['dato']) for key in self.options], allowMultiple=True, defaultValue=[0,1,2,3])
        fdc_scripts.setFlags(fdc_scripts.flags() | QgsProcessingParameterDefinition.FlagAdvanced)
        self.addParameter(fdc_scripts)
